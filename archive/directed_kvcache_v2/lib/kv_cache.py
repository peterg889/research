...
def correct_rope_positions_with_bos(
    ...
    # Pre-compute corrections grouped by theta (usually 1-2 unique values)
    corrections = {}
...
def apply_rope_roundtrip_noise(
    ...
    # Pre-compute forward (+S) and inverse (-S) corrections per theta
    fwd_corrections = {}
    inv_corrections = {}
    for layer_idx in range(len(cache)):
        theta = _get_rope_theta_for_layer(config, layer_idx)
        if theta not in fwd_corrections:
            fwd_corrections[theta] = _build_rope_correction(-offset, head_dim, theta)  # forward +S
            inv_corrections[theta] = _build_rope_correction(offset, head_dim, theta)    # inverse -S

    for layer_idx in range(len(cache)):
        theta = _get_rope_theta_for_layer(config, layer_idx)
        cos_fwd, sin_fwd = fwd_corrections[theta]
        cos_inv, sin_inv = inv_corrections[theta]
        keys = _get_cache_keys(cache, layer_idx)
        device = keys.device
        dtype = keys.dtype
...
def interpolate_values(
    ...
    """
    Blend values: v = alpha * v_primed + (1-alpha) * v_bare. Keys from bare.

    Args:
        bare_cache: Cache providing keys and base values
        primed_cache: Cache providing primed values to blend in
        alpha: Interpolation weight (0.0 = all bare, 1.0 = all primed)

    Returns:
        New DynamicCache with bare keys and interpolated values
    """
...
def replace_values_at_positions(
    bare_cache: Any,
    primed_cache: Any,
    start_pos: int,
    end_pos: int,
    ...
    """
    Replace primed values at token positions [start_pos, end_pos) only. Keys from bare.

    Args:
        bare_cache: Cache providing keys and base values
        primed_cache: Cache providing replacement values
        start_pos: Start position (inclusive)
        end_pos: End position (exclusive)

    Returns:
        New DynamicCache with bare keys and selectively replaced values
    """
    bare_cache = _ensure_dynamic_cache(bare_cache)
    primed_cache = _ensure_dynamic_cache(primed_cache)
    ...
    seq_len = _get_cache_keys(bare_cache, 0).shape[2]
    assert 0 <= start_pos <= end_pos <= seq_len, (
...
def build_cross_doc_cache(
    bare_cache: Any,
    donor_cache: Any,
    target_doc_len: int,
) -> DynamicCache:
    """
    Hybrid cache for cross-document value transfer with length mismatch handling.

    Keys: always from bare_cache.
    Values position 0 (BOS): from bare_cache (preserve BOS).
    For doc positions: take min(target_doc_len, donor_doc_len) from donor,
    pad remainder with bare.

    Args:
        bare_cache: Cache for the current document (keys + fallback values)
        donor_cache: Cache from a different document (donor values)
        target_doc_len: Number of document token positions (excluding BOS)

    Returns:
        New DynamicCache of length 1 + target_doc_len
    """
    bare_cache = _ensure_dynamic_cache(bare_cache)
    donor_cache = _ensure_dynamic_cache(donor_cache)

    n_layers = len(bare_cache)
    assert len(donor_cache) == n_layers, "Caches must have same number of layers"

    # Donor doc length (excluding BOS at position 0)
    donor_seq_len = _get_cache_keys(donor_cache, 0).shape[2]
    donor_doc_len = donor_seq_len - 1  # subtract BOS

    # How many doc positions to copy from donor
    copy_len = min(target_doc_len, donor_doc_len)
    ...
    for layer_idx in range(n_layers):
        ...
        # Values: BOS (pos 0) stays from bare, doc positions from donor
        if copy_len > 0:
            v_donor = _get_cache_values(donor_cache, layer_idx)
            # Donor doc positions are at indices 1..donor_doc_len
            # Target doc positions are at indices 1..target_doc_len
            v[:, :, 1:1 + copy_len, :] = v_donor[:, :, 1:1 + copy_len, :]
...
def _get_text_config(config):
    """Get the text model config, handling composite (multimodal) configs.

    Gemma 3 is loaded as Gemma3ForConditionalGeneration with a composite
    Gemma3Config. The text-model attributes (hidden_size, head_dim, layer_types,
    rope_parameters) live on config.text_config, not the top-level config.
    For Mistral and other pure-text models, config itself has these attributes.
    """
    return getattr(config, 'text_config', config)


def _get_head_dim(config) -> int:
    """Get head_dim, preferring explicit config field over computation.

    Gemma 3 has head_dim=256 which differs from hidden_size//num_attention_heads=320.
    Mistral doesn't set head_dim explicitly, so we fall back to computation.

    Handles composite (multimodal) configs by resolving to text_config first.
    """
    config = _get_text_config(config)
    return getattr(config, 'head_dim', config.hidden_size // config.num_attention_heads)


def _get_rope_theta_for_layer(config, layer_idx: int) -> float:
    """Get rope_theta for a specific layer.

    Handles Gemma 3's per-layer-type rope: config.layer_types + config.rope_parameters.
    Falls back to uniform theta for models like Mistral.

    Handles composite (multimodal) configs by resolving to text_config first.
    """
    config = _get_text_config(config)
    if hasattr(config, 'layer_types') and hasattr(config, 'rope_parameters'):
...
def correct_rope_positions(
    ...
    for layer_idx in range(len(cache)):
        ...
        _set_cache_keys(cache, layer_idx, new_keys)
