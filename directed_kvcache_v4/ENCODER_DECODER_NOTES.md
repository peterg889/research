# Encoder-Decoder KV Cache Enrichment — Experiment Notes

## Overview

Production-realistic test (Exps 01-10): does surrogate-enriched encoder caching still
help when the decoder already has the query as input? Also includes Prefix LM experiments
(Exps 01-03) testing causal vs bidirectional prefix attention on decoder-only models.

## Models

- **T5Gemma 2 4B-4B**: Encoder-decoder (Exps 01-10). See `directed_kvcache_v3/CLAUDE.md`.
- **Gemma 3 12B-IT** (`google/gemma-3-12b-it`): Prefix LM Exps 01-03. BF16, single GPU.
...
```
experiments/encoder_decoder/
  01/ 02/ 03/ 04/ 05/ 06/ 07/ 08/ 09/ 10/

experiments/prefix_lm/
  01/ 02/ 03/
...
1. **Bidirectional attention is catastrophic (d=-0.727, ***)**. Gemma 3 was trained
   ...
   prefix_lm_bare NLL is 4.98 vs causal_bare 2.96.
...
2. **Causal prefixes provide strong enrichment (d≈+0.46, all ***)**. Under standard
   ...
   a strong benefit — comparable to encoder-decoder enrichment.

3. **Almost entirely structural**: random d=+0.475 ≥ oracle d=+0.452. No semantic
   content needed — any tokens prepended before the document help.
...
5. **prefix_lm_oracle_trunc_nq is WORSE than causal_bare_nq (d=-0.579, ***)**:
   Even the strongest prefix_lm condition can't compensate for bidirectional damage.

**Mechanistic interpretation**: The enrichment effect transfers to decoder-only models
