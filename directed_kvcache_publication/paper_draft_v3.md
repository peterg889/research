...
> **⚠️ SUPERSEDED by `paper_draft_v4.md` (2026-06).** The central claim below — that
> TF-IDF keyword prefixes outperform instructions — was measured by absolute NLL and does
> NOT survive an entropy-invariant contrastive-margin re-evaluation (keyword margin effect
> d≈0.00; the NLL gain was confidence inflation, not discrimination). v4 rewrites the paper
> around the corrected measurement, the contrastive-priming result on primable (Gemma)
> models, and the content-routed mechanism. Kept for history.
